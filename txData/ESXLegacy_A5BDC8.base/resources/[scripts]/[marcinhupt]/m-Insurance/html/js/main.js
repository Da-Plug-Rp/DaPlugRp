$(document).ready(function () {
  window.addEventListener("message", function (event) {
    ...
    if (item.display == "viewRegistration") {
      ...
      $("#1p").text(item.citizenName);
      $("#2p").text(item.insuranceDate);
      $("#3p").text(item.insuranceExpire);
      $("#healthinsurance").fadeIn();
    } else if (item.display == "viewHomeInsurance") {
      $("#1c").text(item.citizenName);
      $("#2c").text(item.insuranceDate);
      $("#3c").text(item.insuranceExpire);
