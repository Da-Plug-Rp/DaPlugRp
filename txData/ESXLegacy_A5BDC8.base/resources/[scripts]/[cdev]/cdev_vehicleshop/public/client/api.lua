...
ResourceAPI.GiveKeysToVehicle = function(plate, model, netId)
    ...
    if clib.config.isQBCore then
        if clib.config.public.VehicleKey == "qbcore" then
            TriggerEvent("vehiclekeys:client:SetOwner", plate)
        elseif clib.config.public.VehicleKey == "codesign" then
            TriggerServerEvent("cdev_vehicleshop:giveKeysToVehicleServer", plate, model)
        else
            -- ⚠️ Edit this if you use a custom keys resource
        end
    elseif clib.config.isESX then        
        if clib.config.public.VehicleKey == "codesign" then
            TriggerServerEvent("cdev_vehicleshop:giveKeysToVehicleServer", plate, model)
        else
            -- ⚠️ Edit this if you use a custom keys resource
        end
