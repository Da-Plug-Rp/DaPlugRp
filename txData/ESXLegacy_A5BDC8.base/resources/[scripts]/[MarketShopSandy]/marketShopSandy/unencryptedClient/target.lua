...
if string.lower(Targetting) == "ox" then
...
if string.lower(Targetting) == "qb" then
	-- affError("Targetting set on QB-Target")
	function targetBoxZone(option)
	...
	function targetGlobalObject(option)
		...
		-- GetHashKey("pata_trolleySandy")
		exports['qb-target']:AddTargetModel("pata_trolleySandy", { -- This defines the models, can be a string or a table
		  options = options,
		  distance = 2.5, -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
		})
		return targetID
