...
function ValidateBoxList() {
	...
	if (cptElement > 0 ) {
		$.post('https://marketShopSandy/TakeBoxInStock', JSON.stringify({items : myArray}));
		$('.eachListBoxStock').remove()
	}else{
...
function ValidateQTE() {
	...
	if (tempAction === "order") {
		...
		$.post('https://marketShopSandy/CalculateOrderInfoText', JSON.stringify({item : tempItem, quantity : qte }));
		...
		$.post('https://marketShopSandy/TakeOrPutStock', JSON.stringify({action : tempAction, item : tempItem, quantity : qte }));
	}
}
...
function ValidateINFO() {
	...
	$.post('https://marketShopSandy/OrderForStockValidated', JSON.stringify({item : tempItem, quantity : qte, price : tempPrice }));
...
function DefineProducts(item) {
	...
	$.post('https://marketShopSandy/OpenItemDesc', JSON.stringify({item : tempItem}));
...
function ItemScanned() {
	...
	$('.CashRegRightUpImg').fadeIn(1)
	$('.CashRegRightUpName').fadeIn(1)
	$.post('https://marketShopSandy/ItemScanned', JSON.stringify({item : cashRegListItem[cashRegCurrentIndex].name, quantity : cashRegListItem[cashRegCurrentIndex].qte , cashRegHandle : cashRegCurrentHandle, typeCashReg : cashRegCurrentType}));
...
function CashRegTicket(item,handle){
	if (antiSpam == false) {
		antiSpam = true
		$.post('https://marketShopSandy/TicketPrint', JSON.stringify({handle : cashRegCurrentHandle, typeCashReg : cashRegCurrentType}));
		setTimeout(function(){
			antiSpam = false
		}, 1500)
	} else {
...
function GetDetailsItemDescInShop(itemId){
	...
	$.post('https://marketShopSandy/GetDetailsItemDescInShop', JSON.stringify({itemId,tempidStall}));
}
...
function ProductTake(id,name) {
	...
	$.post('https://marketShopSandy/TakeItemInShell', JSON.stringify({item : item, quantity : qte, aisle : aisleText, shelve : shelveText, typeStall : typeStall }));
}
...
function RemoveFromStall(data) {
	...
	$.post('https://marketShopSandy/RemoveFromStall', JSON.stringify({item : item, aisle : aisle, shelve : shelve}));
}
...
function RemoveFromRegularStall(data) {
	...
	var item = data.id
	aff("aisle: "+aisle+" shelves: "+shelve+" item: "+item)
	$.post('https://marketShopSandy/RemoveFromRegularStall', JSON.stringify({item : item, aisle : aisle, shelve : shelve}));
}
...
function RemoveFromFixedStall(data) {
	...
	$.post('https://marketShopSandy/RemoveFromFixedStall', JSON.stringify({item : item, aisle : aisle}));
}
...
function ValidatePopupStock() {
	...
	$.post('https://marketShopSandy/ValidatePopupStock', JSON.stringify({item : item, aisle : aisle, shelve : shelve, typeAisle : AisleDefineType}));
	CancelPopupStock()
}
...
function CloseAll(){
	...
	// $('.CashRegister').fadeOut(500)
	currentIhmOpen = ""
	$.post('https://marketShopSandy/exit', JSON.stringify({currentIhmOpen : currentIhmOpen}));
}
var Asking = 0
...
// document.getElementById('TradItem').innerHTML = "Itemmmm"
	...
	if (typeof Trad !== 'undefined') {
		...
		Asking = Asking + 1
		if (Asking<3) {
			$.post('https://marketShopSandy/AskForTrad', JSON.stringify({}));
		}
	}
...
function SaveAisle(data){
	...
	$.post('https://marketShopSandy/SaveAisle', JSON.stringify({
...
function SaveRegularAisle(data){
	...
	$.post('https://marketShopSandy/SaveRegularAisle', JSON.stringify({
		AisleId : splittedAisle[1], 
		// AisleSize : AisleSize,
		// AisleColor : AisleColor,
		// ShelveASize: ShelveASize,
...
function SaveFixedAisle(data){
	...
	$.post('https://marketShopSandy/SaveFixedAisle', JSON.stringify({
		AisleId : splittedAisle[1], 
		// AisleSize : AisleSize,
		// AisleColor : AisleColor,
		// ShelveASize: ShelveASize,
...
function DefineAisle(data) {
	...
	$.post('https://marketShopSandy/GetAisleStock', JSON.stringify({id : id}));
}
...
function DefineRegularAisle(data) {
	...
	$.post('https://marketShopSandy/GetRegularAisleStock', JSON.stringify({id : id}));
}
...
function DefineFixedAisle(data) {
	...
	$.post('https://marketShopSandy/GetFixedAisleStock', JSON.stringify({id : id}));
}
...
function AddMenuItemDescInStall(data) {
	if (AisleDefineType == "Configurable") {
		...
		$.post('https://marketShopSandy/AskForADDitemInStall', JSON.stringify({aisle : elmId, shelves : aisle, cat : cat, typeAisle : AisleDefineType}));
