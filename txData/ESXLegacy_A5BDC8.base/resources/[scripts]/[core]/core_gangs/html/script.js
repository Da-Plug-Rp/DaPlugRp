...
function completeRegistration() {
    ...
    var color = $('.register-color').val();
    var picture = $('.register-label').val();
...
function openLeaderboard() {
    ...
    var orgArray = Object.entries(Organizations).map(([key, value]) => {
        var allocation = Object.keys(value.zones).length / Object.keys(Zones).length * 100;
...
function openMenu(mapUnfold = false) {
    ...
    if (Organizations[Organization]) {
        allocation = Object.keys(Organizations[Organization].zones).length / Object.keys(Zones).length * 100;
        allocation = Math.round(allocation * 100) / 100;
