...
function AddMoney(society, amount)
	...
	MySQL.Async.execute('UPDATE okokbanking_societies SET value = value + @value WHERE society = @society OR society = @society2', {
		['@value'] = amount,
...
function GetAccount(society)
	...
	MySQL.Async.fetchAll('SELECT value FROM okokbanking_societies WHERE society = @society OR society = @society2', {
		['@society'] = 'society_'..string.gsub(society, "society_", ""),
		['@society2'] = string.gsub(society, "society_", "")
	}, function(result)
		local db = result[1]
