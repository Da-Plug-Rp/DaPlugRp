		...
		if v.isDone then
			if v.suc then
				if not v.isDis then
					...
					if v.isItem then
						if canCarryIt(v.item, tonumber(v.amount), xPlayer) then
							addItem(xPlayer, v.item, tonumber(v.amount), v.data)
							canCarry = true
						end
					else
						if not hasWeapon(xPlayer, v.item) then
							addWeapon(xPlayer, v.item, tonumber(v.amount))
							canCarry = true
						end
					end
					...
					if canCarry then
						...
						TriggerClientEvent(Config.EventPrefix..':removeItemsFromQueue', xPlayer.source, 1, workbench)
					else
						TriggerClientEvent(Config.EventPrefix..':notification', xPlayer.source, _L('cant_carry').title, _L('cant_carry').text, _L('cant_carry').time, _L('cant_carry').type)
						TriggerClientEvent(Config.EventPrefix..':notification', xPlayer.source, _L('claimed_all_items').title, _L('claimed_all_items').text, _L('claimed_all_items').time, _L('claimed_all_items').type)
						TriggerClientEvent(Config.EventPrefix..':resetClaimAllButton', xPlayer.source)
					end
				else
					local canCarry = true
					for k2,v2 in ipairs(v.recipe) do
						if v2[4] == "false" or v2[4] == false then
							if not canCarryIt(v2[1], tonumber(v2[2]), xPlayer) then
								canCarry = false
							end
						end
					end
					...
					if canCarry then
						for k2,v2 in ipairs(v.recipe) do
							if v2[4] == "true" or v2[4] == true then
								addMoney(v2[1], tonumber(v2[2]), xPlayer)
							else
								addItem(xPlayer, v2[1], tonumber(v2[2]))
							end
						end
