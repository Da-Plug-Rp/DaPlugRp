...
window.addEventListener('message', function (event) {
    ...
    switch (e.type) {
        ...
        case "OpenReception":
            ...
            // LoadNotifys(event.data.notifys);
        ...
        case "LoadElevator":
            ...
            arkasokaklar = event.data.elevator2
            if(event.data.garage){
                LoadElevatorGarage(event.data.floor, event.data.bankx, event.data.garage, event.data.thisIsApartment, event.data.elevator2, event.data.garajelevator)
            } 
            else {
        ...
        case "openRoomList":
            Lang = event.data.lang;
            elevatorMenuIn = true;
            TumOdalar = event.data.rooms;
            localid = event.data.mid;
            Hehe = event.data.playerodalar;
            let elevatorMenu = $('#ElevatorMenu');
            elevatorMenu.html(`
            <div id="MyOdalar" class="ElevatorHaxball">
                <h1>${Lang["MyRoomString"]}</h1>
                <h3>${Lang["mymotelroomsstring"]}</h3>
            </div>`);
            elevatorMenu.fadeIn(250);
            for (let floor = 1; floor <= event.data.rooms.length; floor++) {
                let cam = `
                    <div id="EnterCal" room="${floor}" motel="${localid}" class="ElevatorHaxball">
                        <h1>${Lang["Room"]} ${floor}</h1>
                        <h3>${Lang["KapiyiCal"]}</h3>
                    </div>`;
...
$(document).on('click', '#EnterGir', function(){
    $('#ElevatorMenu').fadeOut(500);
    $.post('http://0r_motels/EnterPlayerRoom', JSON.stringify({
        room: $(this).attr("room"),
        motel: $(this).attr("motel")
    }))
});

$(document).on('click', '#EnterCal', function(){
    $('#ElevatorMenu').fadeOut(500);
    $.post('http://0r_motels/SenCalKapimi', JSON.stringify({
        room: $(this).attr("room"),
        motel: $(this).attr("motel")
    }))
});

$(document).on('click', '#MyOdalar', function(){
    elevatorMenuIn = true;
    let elevatorMenu = $('#ElevatorMenu');
    elevatorMenu.html(`
    <div id="AllRooms" class="ElevatorHaxball">
        <h1>${Lang["AllRoomsString"]}</h1>
        <h3>${Lang["ShowAllRooms"]}</h3>
    </div>`);
    elevatorMenu.fadeIn(250);
    for (let index = 0; index < Hehe.length; index++) {
        const element = Hehe[index];
        let camx = `
            <div id="EnterGir" room="${element.RoomId}" motel="${localid}" class="ElevatorHaxball">
                <h1>${Lang["Room"]} ${element.RoomId}</h1>
                <h3>${Lang["EnterRoom"]}</h3>
            </div>`;
        elevatorMenu.append(camx);
    }
});
...
$(document).on('click', '#AllRooms', function(){
    ...
    elevatorMenu.fadeIn(250);
    for (let floor = 1; floor <= TumOdalar.length; floor++) {
        let cam = `
            <div id="EnterCal" room="${floor}" motel="${localid}" class="ElevatorHaxball">
                <h1>${Lang["Room"]} ${floor}</h1>
                <h3>${Lang["KapiyiCal"]}</h3>
...
OpenBossMenu = function(data, ax, rina){
    ...
    $('#garageayar').hide();
    $('#makbus').show();
    if ((data.apartment === 0) && (data.apartment === 2)){
        $('#coridorayar').hide();
        $('#makbus').css('top', '1rem'); 
    } else {
        $('#makbus').css('top', '1rem'); 
...
LoadMotelOdalar = function(array){
    ...
    for (let index = 0; index < array.length; index++) {
        const element = array[index];
        if (element.Owner !== Lang["NoOwner"] && element.Owner !== '') {
            axx = `
                <div class="Avengers">
    ...
    for (let index = 0; index < data.Colors.length; index++) {
        ...
        if (element.name == LatestTheme) {
            ...
            <div class="SelectT Rog-Item">
                <img src="rooms/${element.name}_${data.Url}.png">
...
function IdentifierConverter(ident){
    ...
    if(ident){
        ...
        } else if(identifier.toLowerCase().includes('steam')){
            dosyaAdi = identifier.replace(/:/g, "_") + ".png";
        } else {
            dosyaAdi = identifier + ".png";
        }
    }
    return dosyaAdi
}
...
function LoadElevatorGarage(floorCount, cenk, bankx, apartment, elevator2, baba) {
    elevatorMenuIn = true;
    let elevatorMenu = $('#ElevatorMenu');
    elevatorMenu.html('');
    elevatorMenu.fadeIn(250);
...
AddMotelButton = function(){
    ...
    $('#ApartmentString67').html(Lang["ApartmentString"]);
    $('#ApartmentString671').html(Lang["ApartmentString2"]);
...
AddMotelRoom = function(){
    ...
    if(secil === 2){
        $('#cemaller').hide();
        $('#apartmant2version').show();
    } else {
        $('#apartmant2version').hide();
        $('#cemaller').show();
    }
    ...
    $('#CreateMotelRoomV2').html(Lang["CreateMotelRoom"]);
    $('#enterinfo').html(Lang["EnterInfo"]);
    $('#TeleportString').html(Lang["TeleportString"])
    $('#EnterRoomCoord2').html(Lang["EnterCoordString"]);
    $('#EnterCoordString').html(Lang["EnterCoordString"]);
    ...
    $('#StashWeight2').html(Lang["StashWeightString"]);
    $('#StashWeightString').html(Lang["StashWeightString"]);
    $('#StashWeightString2').html(Lang["StashWeightString"])
    $('#ApartmentString2').html(Lang["ApartmentString2"]);
    $('#ApartmentString3').html(Lang["ApartmentString3"]);
    $('#RoomPriceString2').html(Lang["RoomPriceString"]);
    $('#motelpricetext').html(Lang["motelpricetext"]);
    $('#TypeText').html(Lang["TypeText"])
    $('#receptioncoords').html(Lang["receptioncoords"]);
    $('#GarageString').html(Lang["GarageString"]);
    ...
    $('#DoorHash').html(Lang["DoorHash"]);
    $('#Price2').html(Lang["RoomPriceString"]);
    $('#ApartmentString5').html(Lang["ApartmentString"]);
    $('#TypeTextString').html(Lang["TypeText"])
    $('#GarageCoordsString').html(Lang["GarageCoordsString"]);
    $('#TypeText').html(Lang["TypeText"])
    $('#MloString').html(Lang["MloString"])
...
$(document).on('change', '#thisApartment input[type="checkbox"]', function() {
    ...
    $('#thisApartment2 input[type="checkbox"]').prop('checked', !apartmentSelected);
    if (apartmentSelected === true) {
        $('#CorridorTheme').fadeOut(350);
    } else {
        $('#CorridorTheme').fadeIn(350).css("display", "flex");
    }
});

$(document).on('change', '#thisApartment2 input[type="checkbox"]', function() {
    corridorSelected = $(this).prop('checked');
    $('#thisApartment input[type="checkbox"]').prop('checked', !corridorSelected);
    if (corridorSelected === true) {
        $('#CorridorTheme').fadeIn(350).css("display", "flex");
    } else {
        $('#CorridorTheme').fadeOut(350);
    }
...
$(document).on('change', '#thisDisabled input[type="checkbox"]', function() {
    ...
    $.post('http://0r_motels/DisabledMotel', JSON.stringify({
...
$(document).ready(function() {
    $("#RoomCount").on('input change', function() {
        var roomPriceInputValue = $(this).val();

        // Girilen değeri kontrol et
        try {
            var sayi = parseInt(roomPriceInputValue);
            if (sayi >= 12 && sayi % 12 === 0) {
                verdimGitti = true;
            } else {
                $.post('http://0r_motels/JsNotify', JSON.stringify({
                    type: "error",
                    title: Lang["Invalid"],
                    text: 'If there is a garage in your apartment system, the number of rooms you create must be a multiple of 12.',
                    time: 3
                }));                
                verdimGitti = false;
            }
...
$(document).on('click', '#CreateRoomPart2', function(){
    var inputText1 = $("#RoomCount").val();
    var inputText2 = $("#RoomPrice5").val();
    var inputText3 = $("#RoomStas").val();
    ...
    if (inputText1 !== "" && inputText2 !== "" && inputText3 !== ""){
        $('.NewEditMenu').fadeOut(350);
        $.post('http://0r_motels/CreateMotelRoomTeleportVer', JSON.stringify({
            roomcount: inputText1,
            price: inputText2,
            weightnew: inputText3,
            roomtype: odaInterior,
            mcode: SecilenMotelMCode
