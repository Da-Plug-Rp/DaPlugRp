...
function GetDatabaseStock()
	...
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstock')
	if r then
		-- aff("r not nil")
		-- if #r > 0 then
...
function Definesellingprice(Item,Price)
	...
	local r = MySQL.Sync.fetchAll('SELECT * from pata_marketshopstock WHERE name = ?',{Item})
	if r then
		-- aff("r")
		if #r > 0 then
		-- aff("#r > 0")
			...
			MySQL.update.await('UPDATE pata_marketshopstock SET sellingprice = ? WHERE name = ? ', {Price, Item}, function(affectedRows)
...
function AddDatabaseStock(Item,Qte)
	...
	if r then
		...
		-- aff("#r > 0")
			...
			MySQL.update.await('UPDATE pata_marketshopstock SET quantity = ? WHERE name = ? ', {newQte, Item}, function(affectedRows)
				if affectedRows then
					-- aff("done : "..affectedRows)
				else 
					-- aff("not done")
			...
			-- aff("#r < 0 ---")
			MySQL.insert('INSERT INTO pata_marketshopstock (name, quantity) VALUES (?, ?) ', {Item, Qte}, function(id)
			-- aff("created: "..id)
			end)
		end
	else
...
function GetDatabaseStall()
	...
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstall')
	if r then
		-- aff("r not nil")
		-- if #r > 0 then
			for k,v in pairs(r) do
...
function GetDatabaseSpecificStall(id)
	...
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstall WHERE id = ?',{StallId})
	if r then
		-- aff("r not nil")
		local data = r[1]
...
function GetDatabaseSpecificRegularStall(id)
	...
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstallnoconf WHERE id = ?',{StallId})
	if r then
		-- aff("r not nil")
		local data = r[1]
		currentStallData = data
...
function GetDatabaseSpecificFixedStall(id)
	...
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstallregnoconf WHERE id = ?',{StallId})
	if r then
		-- aff("r not nil")
		local data = r[1]
		currentStallData = data
...
function GetStockSpecificRegularStall(id,shelves)
	local itemIn = {}
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstallnoconf WHERE id = ?',{id})
...
local itemIn = {}
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstallregnoconf WHERE id = ?',{id})
...
function GetStockSpecificStall(id,shelves)
	local itemIn = {}
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstall WHERE id = ?',{id})
	if r then
		local data = r[1]
		if shelves == "a" or shelves == "A" then
			itemIn = convertItemInToLua(data.AitemIn)
...
function GetDatabaseAllFixedStallConfig(id)
	...
	-- local StallId = id
	local itemIn = {}
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstallregnoconf')
	if r then
		-- aff("r not nil")
		for k,v in pairs(r) do
			local data = v
...
function GetDatabaseAllRegularStallConfig(id)
	...
	-- local StallId = id
	local itemIn = {}
	local r = MySQL.Sync.fetchAll('SELECT * FROM pata_marketshopstallnoconf')
	if r then
		-- aff("r not nil")
		for k,v in pairs(r) do
			local data = v
...
function DefineDatabaseStall(StallData,param)
	...
	if r then
		...
		-- aff("#r > 0")
			...
			MySQL.update.await('UPDATE pata_marketshopstall SET color = ?, sizeOf = ? WHERE id = ? ', {stallType,stallModel, StallId}, function(affectedRows)
				if affectedRows then
					aff("done : "..affectedRows)
				else 
					aff("not done")
...
function DefineDatabaseSpecificStall(StallId,model,typeOf,whichOne)
	...
	if r then
		...
		-- aff("#r > 0")
			...
			if whichOne == "a" then
				MySQL.update.await('UPDATE pata_marketshopstall SET AtypeOf = ?, Amodel = ? WHERE id = ? ', {stallType,stallModel, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
			...
			elseif whichOne == "b" then
				MySQL.update.await('UPDATE pata_marketshopstall SET BtypeOf = ?, Bmodel = ? WHERE id = ? ', {stallType,stallModel, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
			...
			elseif whichOne == "c" then
				MySQL.update.await('UPDATE pata_marketshopstall SET CtypeOf = ?, Cmodel = ? WHERE id = ? ', {stallType,stallModel, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
			...
			elseif whichOne == "d" then
				MySQL.update.await('UPDATE pata_marketshopstall SET DtypeOf = ?, Dmodel = ? WHERE id = ? ', {stallType,stallModel, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
...
function DefineDatabaseSpecificRegularStall(StallId,typeOf,whichOne)
	...
	if r then
		...
		-- aff("#r > 0")
			...
			-- end
			if whichOne == "a" then
				MySQL.update.await('UPDATE pata_marketshopstallnoconf SET AtypeOf = ? WHERE id = ? ', {stallType, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
			...
			elseif whichOne == "b" then
				MySQL.update.await('UPDATE pata_marketshopstallnoconf SET BtypeOf = ? WHERE id = ? ', {stallType, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
			...
			elseif whichOne == "c" then
				MySQL.update.await('UPDATE pata_marketshopstallnoconf SET CtypeOf = ? WHERE id = ? ', {stallType, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
			...
			elseif whichOne == "d" then
				MySQL.update.await('UPDATE pata_marketshopstallnoconf SET DtypeOf = ? WHERE id = ? ', {stallType, StallId}, function(affectedRows)
					if affectedRows then
						aff("done : "..affectedRows)
					else 
						aff("not done")
...
function DefineDatabaseSpecificFixedStall(StallId,typeOf)
	...
	if r then
		...
		aff("#r > 0")
			...
			MySQL.update.await('UPDATE pata_marketshopstallregnoconf SET typeOf = ? WHERE id = ? ', {stallType, StallId}, function(affectedRows)
				if affectedRows then
					aff("done : "..affectedRows)
				else 
					aff("not done")
...
function AddItemDatabaseStall(id,itemData,shelves)
	...
	if r then
		...
		if shelves == "a" or shelves == "A" then
			...
			MySQL.update.await('UPDATE pata_marketshopstall SET AitemIn = ? WHERE id = ? ', {temp, StallId})
		elseif shelves == "b" or shelves == "B" then
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstall SET BitemIn = ? WHERE id = ? ', {temp, StallId})
		elseif shelves == "c" or shelves == "C" then
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstall SET CitemIn = ? WHERE id = ? ', {temp, StallId})
		elseif shelves == "d" or shelves == "D" then
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstall SET DitemIn = ? WHERE id = ? ', {temp, StallId})
		end
...
function AddItemDatabaseRegularStall(id,itemData,shelves)
	...
	if r then
		...
		if shelves == "a"  or shelves == "A" then
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstallnoconf SET AitemIn = ? WHERE id = ? ', {temp, StallId})
		elseif shelves == "b" or shelves == "B" then
			local currentStock = json.decode(r[1].BitemIn)
			if currentStock then
				for k,v in pairs(currentStock) do
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstallnoconf SET BitemIn = ? WHERE id = ? ', {temp, StallId})
		elseif shelves == "c" or shelves == "C" then
			local currentStock = json.decode(r[1].CitemIn)
			if currentStock then
				for k,v in pairs(currentStock) do
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstallnoconf SET CitemIn = ? WHERE id = ? ', {temp, StallId})
		elseif shelves == "d" or shelves == "D" then
			local currentStock = json.decode(r[1].DitemIn)
			if currentStock then
				for k,v in pairs(currentStock) do
			...
			temp = json.encode(currentStock)
			MySQL.update.await('UPDATE pata_marketshopstallnoconf SET DitemIn = ? WHERE id = ? ', {temp, StallId})
		end
...
function AddItemDatabaseFixedStall(id,itemData)
	...
	if r then
		...
		temp = json.encode(currentStock)
		MySQL.update.await('UPDATE pata_marketshopstallregnoconf SET itemIn = ? WHERE id = ? ', {temp, StallId})
...
-- TriggerServerEvent("marketShop:TakePutInStock",item,action,qte)
...
RegisterCommand('AddDatabaseStock', function(source)
	...
	-- TriggerServerEvent("marketShop:GetStockDetails")
	-- TriggerEvent("marketShop:OpenUi","Close")
end)
...
RegisterCommand('GetDatabaseAllStall', function()
	...
	TriggerClientEvent("marketShop:SendAllStallConfig",player,t)
	-- TriggerServerEvent("marketShop:GetStockDetails")
	-- TriggerEvent("marketShop:OpenUi","Close")
end)
