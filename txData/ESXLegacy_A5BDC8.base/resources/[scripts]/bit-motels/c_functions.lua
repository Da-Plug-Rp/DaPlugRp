...
function openDeposit()
    if Config.Framework == "esx" then
        ...
        TriggerEvent('ox_inventory:openInventory', 'stash', "motelstash_" .. ESX.GetPlayerData().identifier)
    else
        TriggerServerEvent("inventory:server:OpenInventory", "stash",
            "motelstash_" .. QBCore.Functions.GetPlayerData().citizenid)
...
function startWashFace()
    ...
    TaskPlayAnim(PlayerPedId(), "missmic2_washing_face", "michael_washing_face", 2.0, 2.0, 5000, 0, 0, false, false,
        false)
...
function startShower()
    ...
    TaskPlayAnim(PlayerPedId(), "anim@mp_player_intmenu@key_fob@", "fob_click", 2.0, 2.0, 2500, 0, 0, false, false,
        false)
...
function startSleep()
    ...
    local entityCam = {
        use = false,
        ped = nil,
        coords = Config.sleepCameraCoord,
        distance = nil,
        rotation = 330,
        fovpoint = nil
    }
...
function useDrawText(text)
    drawTxt(text, Config.alertTextFont, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
end

function useDraw3DText(text, coord)
    Draw3DText(coord.x, coord.y, coord.z - 1.5, text, 4, 0.15, 0.07)
end
